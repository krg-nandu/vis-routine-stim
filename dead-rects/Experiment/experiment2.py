...
res_folder = './results2/%s/' % name

n_trials=input('How many trials per condition do you want to do today?')

border = input('Shall I display borders? [anything besides 0 is Yes]')
border = bool(border)
...
def Trial(window,clock,num_colors=8,positions=[],border=False,exponent=3):    
...
movieText = visual.TextStim(window, text='Dear Participant,\n\n' +
                            'Before we start with a set of images, we want to show you \n'+
                            'how the images used in that set are generated.\n'+
...
readyText = visual.TextStim(window, text='Dear Participant,\n\n' +
                            ...
                            'if they do not press z.\n\n'+
                            'Only the first press counts and you will see every image for a second.\n\n'+
...
resultsAll = np.zeros((0,7))
# results = [exponent,n_colours,distance,angle,abs_angle,truth,response]
...
for iBlock in range(len(exponents)):
    ...
    for i in range(len(results)):
        ...
        resList.append(Trial(window,clock,num_colors=n_c,positions=pos,border=border,exponent=exponent))
    ...
    if resList[i][2][0][0]=='q':
        break
    results = np.append(exponent*np.ones((results.shape[0],1)),results,axis=1)# add exponent
